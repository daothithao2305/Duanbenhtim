...
try:
    ...
    st.warning("⚠️ Chưa tìm thấy file. Hãy đảm bảo 2 file 'best_svm_model.pkl' và 'scaler.pkl' nằm cùng thư mục với file app.py!")
...
# ---> ĐÃ THÊM: Ô NHẬP TÊN BỆNH NHÂN <---
patient_name = st.text_input("Tên bệnh nhân", placeholder="Nhập họ và tên của bạn...")
...
if st.button("✨ XEM KẾT QUẢ DỰ ĐOÁN"):
    ...
    # Xử lý tên: Nếu ô tên trống, mặc định gọi là "Bạn"
    name_display = patient_name.strip() if patient_name.strip() else "Bạn"
    ...
    # HIỂN THỊ KẾT QUẢ CÓ KÈM TÊN BỆNH NHÂN
    if ket_qua == 1:
        # Giao diện KHI CÓ BỆNH 
        st.error(f"### ⚠️ CẢNH BÁO: BỆNH NHÂN {name_display.upper()} CÓ NGUY CƠ MẮC BỆNH CAO!")
        st.write(f"Dựa trên thuật toán, chúng tôi phát hiện những dấu hiệu bất thường về tim mạch của **{name_display}**.")
        ...
        st.success(f"### 🎉 CHÚC MỪNG {name_display.upper()}! KHÔNG CÓ NGUY CƠ!")
        st.write(f"Các chỉ số đều nằm trong vùng an toàn. Trái tim của **{name_display}** đang rất khỏe mạnh!")
